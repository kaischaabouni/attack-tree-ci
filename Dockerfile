...
RUN apt update && \
    ...
    apt install -yq /attack-tree/${MODELIO_PKG_NAME}.deb
    
## The following command would create a virtual display with Xvfb and then generate files according to the project setting
CMD Xvfb :1 -screen 0 1024x768x16 & DISPLAY=:1.0 modelio-open-source3.8 -consoleLog -workspace /attack-tree/workspace/ -project test_AttackTrees5 -batch /attack-tree/jython_scripts/script1.py 